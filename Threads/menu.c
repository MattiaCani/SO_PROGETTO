...
pthread_t tTesto, tUfo, tMusica, tStelle;
pos oggetto;
...
void generaStelle(WINDOW *finestra){
    ...
    if(pthread_create(&tStelle, NULL, disegnaStelle(finestra), NULL)){
        endwin();
        exit;
    }
}
...
void crediti(){
    ...
    mvprintw(16, 30, "SPACE DEFENDER v1.1 [Threads Version]");
...
void opening(){
    ...
    if(pthread_create(&tMusica, NULL, &musica, NULL)){
        endwin();
        exit;
    }

    if(pthread_create(&tTesto, NULL, &welcome, NULL)){
        endwin();
        exit;
    }

    if(pthread_create(&tUfo, NULL, &ufo, NULL)){
        endwin();
        exit;
    }
